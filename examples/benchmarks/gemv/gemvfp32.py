...
class GemvFP32Kernel(NdpKernel):
  def __init__(self, num_m2ndps=1):
    super().__init__()
    self.num_m2ndp = num_m2ndps
    self.vector_size = 14336
    self.output_size = 7168
    ...
    self.sync = 0
    self.kernel_id = 0
    self.kernel_name = f'gemvfp32_kernel'
    self.base_addrs = self.weight_addr
    self.bound = int(self.vector_per_m2ndp * self.output_size * configs.data_size * self.batch_size / 8)
    ...
    self.partial_output = np.zeros((self.batch_size, 1, self.output_size)).astype(np.float32)
    self.partial_acc_output.append(self.partial_output.copy())
    for i in range(num_m2ndps):
      self.partial_output += (self.input[:, :, i * self.vector_per_m2ndp : (i + 1) * self.vector_per_m2ndp] @ self.weight[:, i * self.vector_per_m2ndp : (i + 1) * self.vector_per_m2ndp, :])
      self.partial_acc_output.append(self.partial_output.copy())
  ...
  def make_kernel(self):
    ...
    for i in range(self.unroll_level):
      template += f'vamoaddei32.v x0, (x2), v31, v{i + 1}\n'
      template += f'addi x2, x2, {configs.packet_size}\n'
    template += f'addi x6, x6, {8 * self.unroll_level}\n' # 256 / float 32 = 8 elements in a row
