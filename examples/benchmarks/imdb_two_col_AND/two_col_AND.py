...
class TwoColANDKernel(NdpKernel):
    def __init__(self):
        super().__init__()
        self.packet_size = 64
        self.int64_size = 8
    ...
    def make_kernel(self):
        ...
        template += f'vle64.v v2, (x3)\n'
        template += f'li x4, {spad_addr + 8}\n'
        template += f'ld x4, (x4)\n' #x4 = output_addr
        template += f'add x4, x4, x2\n'
        template += f'vand.vv v3, v1, v2\n'
        template += f'vse64.v v3, (x4)\n'

        return template

    def make_input_map(self):
      return make_memory_map([(self.input_a_addr, self.input_a), (self.input_b_addr, self.input_b)], packet_size=self.packet_size)
