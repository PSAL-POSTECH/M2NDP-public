...
import numpy as np
import os
from utils.utils import MultipleKernel, make_memory_map, pad8
import configs

# Reduce Max Kernel for SoftMax 

class MultiSoftMaxKernel0(MultipleKernel):
  def __init__(self, repeat=17):
    ...
    self.sync = 0
    self.kernel_id = 0
    self.kernel_name = f'softmax_multi_kernel0'
    ...
    input_data = np.random.rand(repeat,self.vector_size).astype(data_type)#np.random.rand(self.vector_size).astype(np.float32)
