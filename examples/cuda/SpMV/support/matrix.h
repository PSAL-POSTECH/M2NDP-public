...
static struct COOMatrix readCOOMatrix(const char* fileName) {
    ...
    if (!fp.is_open()) {
        PRINT_ERROR("Error opening matrix file %s", fileName);
        exit(1);
    }
    fp >> cooMatrix.numRows;
    ...
    fp >> cooMatrix.numCols;
    fp >> cooMatrix.numNonzeros;
    ...
    for(uint32_t i = 0; i < cooMatrix.numNonzeros; ++i) {
        ...
        if (!std::getline(fp, line)) {
            PRINT_ERROR("Error reading matrix file %s", fileName);
            exit(1);
        }
        std::istringstream iss(line);
        iss >> cooMatrix.rowIdxs[i] >> cooMatrix.nonzeros[i].col;
        if(iss.fail()) {
            cooMatrix.nonzeros[i].value = 1.0f;
        } else {
            iss >> cooMatrix.nonzeros[i].value;
        }
        ...
        // if (fscanf(fp, "%f", &value) == 1) {
            // cooMatrix.nonzeros[i].value = value;
        // } else {
            // cooMatrix.nonzeros[i].value = 1.0f;
        // }
    }
