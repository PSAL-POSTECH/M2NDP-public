...
protected:
    ...
    // ScalarStat ctrl_bytes;

    // total cycle
    ScalarStat total_cycle;

    ScalarStat precharges;
    ScalarStat precharge_alls;
    ScalarStat activations;
    ScalarStat refreshes;
    ScalarStat row_hits;
...
public:
        ...
        cmd_trace_files(channel->children.size())
    {
        ...
        // regStats
        total_cycle
            .name("Total_simulation_cycles_"+to_string(channel->id) + "_core")
            .desc("Total simulation cycle")
            .precision(0)
            ;
        precharges
            .name("precharges_channel_"+to_string(channel->id) + "_core")
            .desc("Number of PRE commands per channel")
            .precision(0)
            ;
        precharge_alls
            .name("precharge_alls_channel_"+to_string(channel->id) + "_core")
            .desc("Number of PREA commands per channel")
            .precision(0)
            ;
        activations
            .name("activations_channel_"+to_string(channel->id) + "_core")
            .desc("Number of ACT commands per channel")
            .precision(0)
            ;
        refreshes
            .name("refreshes_channel_"+to_string(channel->id) + "_core")
            .desc("Number of REF commands per channel")
            .precision(0)
            ;
        row_hits
    ...
    int get_precharges() {
      return precharges.value() +
             precharge_alls.value() * 
             channel->spec->org_entry.count[2] * channel->spec->org_entry.count[3]; // Bank * Bank group
    }
    ...
    void tick()
    {
        ...
        if (req->is_first_command) {
            ...
            //   ctrl_bytes += req->mf->get_ctrl_size();
        ...
        if (req->type == Request::Type::R_WRITE) {
            channel->update_serving_requests(req->addr_vec.data(), -1, clk);
            // Write request can incoporate multiple requests,
            // because the size of the write requst ranges from 32B to 128B
            if (req->hasNextAddr()) {
                req->is_first_command = true;
                req->addr = req->getNextAddr();
                // addrdec_t raw_addr;
                // addrdec_tlx() must receive address including channel bits
                // m_config->m_address_mapping.addrdec_tlx(req->addr, &raw_addr);
                // set_memory_addr(req, raw_addr);
...
private:
    ...
    void issue_cmd(typename T::Command cmd, const vector<int>& addr_vec)
    {
        ...
        if (cmd_name == "ACT" || cmd_name == "ACT1" || cmd_name == "ACT2")
            activations++;
        else if (cmd_name == "PRE")
            precharges++;
        else if (cmd_name == "PREA")
            precharge_alls++;
        else if (cmd_name == "REF" || cmd_name == "REFab" || cmd_name == "REFpb")
            refreshes++;
