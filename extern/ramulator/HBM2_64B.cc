...
string HBM2_64B::standard_name = "HBM2_64B";

map<string, enum HBM2_64B::Org> HBM2_64B::org_map = {
    {"HBM2_64B_2Gb", HBM2_64B::Org::HBM2_64B_2Gb},
    {"HBM2_64B_4Gb", HBM2_64B::Org::HBM2_64B_4Gb},
    {"HBM2_64B_8Gb", HBM2_64B::Org::HBM2_64B_8Gb},
};

map<string, enum HBM2_64B::Speed> HBM2_64B::speed_map = {
    {"HBM2_64B_2Gbps", HBM2_64B::Speed::HBM2_64B_2Gbps},
    {"HBM2_64B_1.6Gbps", HBM2_64B::Speed::HBM2_64B_1600Mbps},
    {"HBM2_64B_2.4Gbps", HBM2_64B::Speed::HBM2_64B_2400Mbps},
    {"HBM2_64B_0.8Gbps", HBM2_64B::Speed::HBM2_64B_800Mbps},
    {"HBM2_64B_0.4Gbps", HBM2_64B::Speed::HBM2_64B_400Mbps},
    {"HBM2_64B_1.25Gbps", HBM2_64B::Speed::HBM2_64B_1250Mbps}
};

HBM2_64B::HBM2_64B(Org org, Speed speed)
    : org_entry(org_table[int(org)]),
    speed_entry(speed_table[int(speed)]),
    read_latency(speed_entry.nRL + speed_entry.nBL)
{
    // for (int i = 0; i < 5; i++) {
    //     printf("org_entry.count[%d]: %d\n", i, org_entry.count[i]);
    // }
    init_speed();
    init_prereq();
...
HBM2_64B::HBM2_64B(const string& org_str, const string& speed_str) :
    HBM2_64B(org_map[org_str], speed_map[speed_str])
{
}

void HBM2_64B::set_channel_number(int channel) {
//   printf("set_channel called!!!\n");
  org_entry.count[int(Level::Channel)] = channel;
}

void HBM2_64B::set_rank_number(int rank) {
//   printf("set_rank called!!!\n");
  org_entry.count[int(Level::Rank)] = rank;
}


void HBM2_64B::init_speed()
{
    const static int RFC_TABLE[int(Speed::MAX)][int(Org::MAX)] = {
        {160, 260, 350},
        {int(160*0.877), int(260*0.877), int(350*0.877)},        
        {int(160*1.316), int(260*1.316), int(350*1.316)},
        {int(160*0.439), int(260*0.439), int(350*0.439)},
        {int(160*0.220), int(260*0.220), int(350*0.220)},
        {int(160*0.625), int(260*0.625), int(350*0.625)}     
    };
    ...
    switch (speed_entry.rate) {
        ...
        case 1754: speed = 1; break;
        case 2632: speed = 2; break;
        case 878: speed = 3; break;
        case 440: speed = 4; break;
        case 1250: speed = 5; break;
...
void HBM2_64B::init_prereq()
{
    // RD
    prereq[int(Level::Rank)][int(Command::RD)] = [] (DRAM<HBM2_64B>* node, Command cmd, int id) {
    ...
    prereq[int(Level::Bank)][int(Command::RD)] = [] (DRAM<HBM2_64B>* node, Command cmd, int id) {
        switch (int(node->state)) {
    ...
    // REF
    prereq[int(Level::Rank)][int(Command::REF)] = [] (DRAM<HBM2_64B>* node, Command cmd, int id) {
    ...
    // REFSB
    prereq[int(Level::Bank)][int(Command::REFSB)] = [] (DRAM<HBM2_64B>* node, Command cmd, int id) {
        if (node->state == State::Closed) return Command::REFSB;
        return Command::PRE;};

    // PD
    prereq[int(Level::Rank)][int(Command::PDE)] = [] (DRAM<HBM2_64B>* node, Command cmd, int id) {
        switch (int(node->state)) {
    ...
    // SR
    prereq[int(Level::Rank)][int(Command::SRE)] = [] (DRAM<HBM2_64B>* node, Command cmd, int id) {
        switch (int(node->state)) {
...
void HBM2_64B::init_rowhit()
{
    // RD
    rowhit[int(Level::Bank)][int(Command::RD)] = [] (DRAM<HBM2_64B>* node, Command cmd, int id) {
        switch (int(node->state)) {
...
void HBM2_64B::init_rowopen()
{
    // RD
    rowopen[int(Level::Bank)][int(Command::RD)] = [] (DRAM<HBM2_64B>* node, Command cmd, int id) {
        switch (int(node->state)) {
            case int(State::Closed): return false;
...
void HBM2_64B::init_lambda()
{
    lambda[int(Level::Bank)][int(Command::ACT)] = [] (DRAM<HBM2_64B>* node, int id) {
        node->state = State::Opened;
        node->row_state[id] = State::Opened;};
    lambda[int(Level::Bank)][int(Command::PRE)] = [] (DRAM<HBM2_64B>* node, int id) {
        node->state = State::Closed;
        node->row_state.clear();};
    lambda[int(Level::Rank)][int(Command::PREA)] = [] (DRAM<HBM2_64B>* node, int id) {
        for (auto bg : node->children)
            for (auto bank : bg->children) {
                bank->state = State::Closed;
                bank->row_state.clear();
            }};
    lambda[int(Level::Rank)][int(Command::REF)] = [] (DRAM<HBM2_64B>* node, int id) {};
    lambda[int(Level::Bank)][int(Command::RD)] = [] (DRAM<HBM2_64B>* node, int id) {};
    lambda[int(Level::Bank)][int(Command::WR)] = [] (DRAM<HBM2_64B>* node, int id) {};
    lambda[int(Level::Bank)][int(Command::RDA)] = [] (DRAM<HBM2_64B>* node, int id) {
        node->state = State::Closed;
        node->row_state.clear();};
    lambda[int(Level::Bank)][int(Command::WRA)] = [] (DRAM<HBM2_64B>* node, int id) {
        node->state = State::Closed;
        node->row_state.clear();};
    lambda[int(Level::Rank)][int(Command::PDE)] = [] (DRAM<HBM2_64B>* node, int id) {
        for (auto bg : node->children)
            for (auto bank : bg->children) {
                if (bank->state == State::Closed)
                    continue;
    ...
    lambda[int(Level::Rank)][int(Command::PDX)] = [] (DRAM<HBM2_64B>* node, int id) {
        node->state = State::PowerUp;};
    lambda[int(Level::Rank)][int(Command::SRE)] = [] (DRAM<HBM2_64B>* node, int id) {
        node->state = State::SelfRefresh;};
    lambda[int(Level::Rank)][int(Command::SRX)] = [] (DRAM<HBM2_64B>* node, int id) {
        node->state = State::PowerUp;};
}


void HBM2_64B::init_timing()
{
