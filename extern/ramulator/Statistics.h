...
class StatBase { // wrapper for Stats::DataWrap
  ...
  public:
    ...
    StatBase<StatType> & flags(Stats_NDPSim::Flags _flags) {
      stat.flags(_flags);
      return self();
    }
    ...
    Stats_NDPSim::size_type size(void) const { return stat.size(); }
...
class StatBaseVec : public StatBase<StatType> { // wrapper for Stats::DataWrapVec
  ...
  public:
    StatBaseVec<StatType> & subname(Stats_NDPSim::off_type index, const std::string & name) {
      StatBase<StatType>::stat.subname(index, name);
      return self();
    }

    StatBaseVec<StatType> & subdesc(Stats_NDPSim::off_type index, const std::string & desc) {
...
class ScalarStatBase : public StatBase<StatType> { // wrapper for Stats::ScalarBase
  public:
    Stats_NDPSim::Counter value() const { return StatBase<StatType>::stat.value(); };
...
class VectorStatBase : public StatBaseVec<StatType> { // wrapper for Stats::VectorBase
  ...
  public:
    void value(Stats_NDPSim::VCounter & vec) const { StatBase<StatType>::stat.value(vec); }
    void result(Stats_NDPSim::VResult & vec) const { StatBase<StatType>::stat.result(vec); }
    Stats_NDPSim::Result total(void) const { return StatBase<StatType>::stat.total(); }

    bool check(void) const { return StatBase<StatType>::stat.check(); }

    VectorStatBase<StatType, Element> & init(Stats_NDPSim::size_type size) {
      StatBase<StatType>::stat.init(size);
      return self();
    }
    Element &operator[](Stats_NDPSim::off_type index) { return StatBase<StatType>::stat[index]; }
};
...
class ScalarStat : public ScalarStatBase<Stats_NDPSim::Scalar> {
  public:
    using ScalarStatBase<Stats_NDPSim::Scalar>::operator=;
};

class AverageStat : public ScalarStatBase<Stats_NDPSim::Average> {
  public:
    using ScalarStatBase<Stats_NDPSim::Average>::operator=;
};

class VectorStat : public VectorStatBase<Stats_NDPSim::Vector, Stats_NDPSim::Scalar> {
};

class AverageVectorStat : public VectorStatBase<Stats_NDPSim::AverageVector, Stats_NDPSim::Average> {
};

class DistributionStat : public DistStatBase<Stats_NDPSim::Distribution> {
  protected:
    DistributionStat & self() { return *this; }

  public:
    DistributionStat & init(Stats_NDPSim::Counter min, Stats_NDPSim::Counter max, Stats_NDPSim::Counter bkt) {
      StatBase<Stats_NDPSim::Distribution>::stat.init(min, max, bkt);
      return self();
    }

};

class HistogramStat : public DistStatBase<Stats_NDPSim::Histogram> {
  protected:
    HistogramStat & self() { return *this; }

  public:
    HistogramStat & init(Stats_NDPSim::size_type size) {
      StatBase<Stats_NDPSim::Histogram>::stat.init(size);
      return self();
    }
};

class StandardDeviationStat : public DistStatBase<Stats_NDPSim::StandardDeviation> {
};

class AverageDeviationStat : public DistStatBase<Stats_NDPSim::AverageDeviation> {
};
